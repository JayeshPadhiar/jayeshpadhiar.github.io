	...
	return (
		<MainContent>
			<section id="books" className="w-full flex flex-col items-start justify-start gap-8 py-16 mx-auto">
				...
				{booksData.map((category: { type: string, books: { title: string, author: string, description: string, image: string, status: string, averageRating: number, myRating: number, myReview: string, numberOfPages: number, type: string }[] }) => (
					<div key={category.type} className="flex flex-col items-start justify-start gap-4 w-full mb-8">
						...
						<div className="flex flex-row flex-wrap gap-4 justify-start items-start w-full">
							{category.books.map((book: { title: string, author: string, description: string, image: string, status: string, averageRating: number, myRating: number, myReview: string, numberOfPages: number, type: string }) => (
								<BookCard key={book.title} title={book.title} author={book.author} description={book.description} status={book.status} averageRating={book.averageRating} myRating={book.myRating} myReview={book.myReview} numberOfPages={book.numberOfPages} type={book.type} />
							))}
						</div>
					</div>
				))}
